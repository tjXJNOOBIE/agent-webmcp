...
## Local / No-Account Video Toolchain

This project has a local-first motion-asset and finishing pipeline. Use these tools before hand-authoring equivalent effects. Do not require Canva, Figma, LottieFiles, Runway, or another account-based SaaS unless the user explicitly requests it.

Tool routing order:

1. **Animotion MCP** — first choice for production SVG icons and small reusable CSS/micro-animations. Search/reuse an existing asset before drawing a bespoke icon.
2. **PinePaper MCP** — custom animated SVGs, diagrams, procedural backgrounds, charts, vector callouts, and motion assets that Animotion cannot supply. Export deterministic assets into the project.
3. **HyperFrames** — HTML/CSS/GSAP scenes, product UI motion, overlays, title cards, audio-reactive visuals, and the current Agent WebMCP launch composition.
4. **Remotion** — use when a shot-based React timeline, reusable transition package, or programmatic edit is a better fit than HyperFrames. Prefer `@remotion/transitions` over rebuilding standard transitions manually.
5. **Kinocut** — deterministic local finishing: trims, joins, captions, audio normalization, transcoding, quality checks, receipts, and release checkpoints. It is the preferred FFmpeg-facing layer.

The repo-level `.codex/config.toml` registers Animotion, PinePaper, and Kinocut for MCP clients that honor repository-local MCP configuration. Tavall-managed Codex may not inject that configuration, so `scripts/mcp-client.mjs` remains the shell-accessible bridge. Run `npm run tools:bootstrap` once on a machine, then `npm run tools:doctor` before a generation pass. The bootstrap installs the pinned Node dependencies, an ARM-native Playwright Chromium under the ignored `.browser-cache/`, and Kinocut's pinned Python/runtime cache under the ignored `.video-tools/`. Puppeteer's own browser download is disabled; PinePaper and HyperFrames share the Playwright browser path.

Asset policy:

- Reuse generated assets when they already satisfy the shot; do not regenerate for novelty.
- Prefer vector/HTML motion for UI overlays and diagrams so assets remain crisp at 1080p/4K.
- Keep generated intermediate assets under `assets/generated/` and source captures under `assets/source/` when those directories are introduced.
- Every asset used in the final timeline must have a deterministic local source. No render-time network fetches.
- Do not replace supplied narration or music unless explicitly requested. Audio remains the pacing source when the brief says so.
- Run the HyperFrames check plus Kinocut quality/release checks before calling a final export complete.

See `VIDEO_PIPELINE.md` for the complete routing and validation flow.
