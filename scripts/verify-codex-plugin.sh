...
assert plugin.get('version') == '0.3.1', plugin
...
assert 'mcpServers' not in plugin, 'direct MCP declaration would make the web plugin Desktop only'
...
assert match.get('version') == '0.3.1', match
PY
...
skill=$(find "$CODEX_HOME/plugins/cache/agent-webmcp/agent-webmcp/0.3.1" -path '*/skills/agent-webmcp/SKILL.md' -print -quit)
test -n "$skill"
...
echo "CODEX_PLUGIN_VERSION=0.3.1"
