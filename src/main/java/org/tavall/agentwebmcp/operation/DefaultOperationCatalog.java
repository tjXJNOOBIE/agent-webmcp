...
public final class DefaultOperationCatalog {
    ...
    public static OperationCatalog create() {
        ...
        register(catalog, "system.status", "Read Agent WebMCP runtime, provider, catalog, and authentication-mode status.", OperationAccess.READ_ONLY, EmptyInput.class, new SystemStatusOperation());
        ...
        register(catalog, "service.list", "List services on the selected target using the authoritative service provider.", OperationAccess.READ_ONLY, ServiceListInput.class, new ServiceListOperation());
        register(catalog, "service.inspect", "Inspect authoritative state and bounded runtime metadata for one service.", OperationAccess.READ_ONLY, ServiceIdInput.class, new ServiceInspectOperation());
        register(catalog, "service.status", "Read the concise authoritative lifecycle status for one service.", OperationAccess.READ_ONLY, ServiceIdInput.class, new ServiceStatusOperation());
        register(catalog, "service.logs", "Read bounded recent service logs and return a continuation cursor when available.", OperationAccess.READ_ONLY, ServiceLogsInput.class, new ServiceLogsOperation());
        register(catalog, "service.start", "Start one service and return provider-observed lifecycle state.", OperationAccess.MUTATING, ServiceIdInput.class, new ServiceStartOperation());
        register(catalog, "service.stop", "Stop one service and return provider-observed lifecycle state.", OperationAccess.MUTATING, ServiceIdInput.class, new ServiceStopOperation());
        register(catalog, "service.restart", "Restart one service and return provider-observed lifecycle state.", OperationAccess.MUTATING, ServiceIdInput.class, new ServiceRestartOperation());
        register(catalog, "service.reload", "Request a provider-supported service reload and return observed lifecycle state.", OperationAccess.MUTATING, ServiceIdInput.class, new ServiceReloadOperation());
        register(catalog, "job.list", "List bounded durable operation jobs known to this Agent WebMCP runtime.", OperationAccess.READ_ONLY, JobListInput.class, new JobListOperation());
        register(catalog, "job.inspect", "Inspect one durable operation job, including its final canonical operation result when available.", OperationAccess.READ_ONLY, JobIdInput.class, new JobInspectOperation());
        ...
        register(catalog, "job.execute", "Execute one catalog operation as a bounded durable job.", OperationAccess.MUTATING, JobExecuteInput.class, new JobExecuteOperation());
        return catalog;
    }
