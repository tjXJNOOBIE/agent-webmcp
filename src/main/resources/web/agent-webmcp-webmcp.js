(() => {
  ...
  async function executeOperation(operationId, input) {
    ...
    if (!response.ok) {
      const message = payload?.error?.message ?? `Operation ${operationId} failed with HTTP ${response.status}`;
  ...
  async function register() {
    ...
    if (!response.ok) {
      throw new Error(`Unable to load operation catalog: HTTP ${response.status}`);
    }
    const payload = await response.json();
    ...
    runtime.state = 'ready';
    document.dispatchEvent(new CustomEvent('agent-webmcp:ready', {
      detail: { operationCount: runtime.registeredOperationIds.length }
    }));
  }
  ...
  register().catch((error) => {
    ...
    console.error('[Agent WebMCP] WebMCP registration failed', error);
  });
})();
